...
public class Trivia
{
 ...
 // ID of the reward
 private static int _rewardID = 15001;
 ...
 {
 pi.sendPacket(new CreatureSay(12345, Say2.TELL, "Trivia", "Correto!"));
 setState(EventState.REWARDING);
 Announcements.getInstance().gameAnnounceToAll("O vencedor eh " + pi.getName() + "! Ele respondeu em " + (System.currentTimeMillis() - questionTime) / 1000 + " segundos!");
 announceCorrect();
 pi.addItem("Trivia", _rewardID, _rewardCount, pi, true);
 }
 else
 pi.sendPacket(new CreatureSay(1234, 2, "Trivia", "Resposta Errada."));

 }

 public static void startTrivia()
 {
 Announcements.getInstance().gameAnnounceToAll("Trivia Event comecou! Voce tem que enviar PM Trivia com a sua resposta , prepare-se!");
 ...
 {
 pickQuestion();
 Announcements.getInstance().gameAnnounceToAll("Pergunta: " + question);
 ...
 Announcements.getInstance().gameAnnounceToAll("A resposta correta seria: " + answer);
 asked++;
 setState(EventState.ASKING);
 }
